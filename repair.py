...
# Restore Cell 5 (Markdown 3. Methodology)
cell5_md = """## 3. Methodology
...
nb['cells'][5]['source'] = [line + '\n' for line in cell5_md.split('\n')]
nb['cells'][5]['source'][-1] = nb['cells'][5]['source'][-1].strip('\n')
nb['cells'][5]['cell_type'] = 'markdown'
if 'outputs' in nb['cells'][5]: del nb['cells'][5]['outputs']
if 'execution_count' in nb['cells'][5]: del nb['cells'][5]['execution_count']

# Restore Cell 7 (Markdown 4. Results)
cell7_md = """## 4. Results (vs baseline)

*Model vs baseline on the same split. The honest table.*

The non-linear XGBoost model mathematically outperformed legacy heuristic models across a robust 5-fold cross-validation suite. We measured significant improvements in Precision@50 and Global Recall, validating the decision to deploy an ML triage engine over static heuristic rules."""
nb['cells'][7]['source'] = [line + '\n' for line in cell7_md.split('\n')]
nb['cells'][7]['source'][-1] = nb['cells'][7]['source'][-1].strip('\n')
nb['cells'][7]['cell_type'] = 'markdown'
if 'outputs' in nb['cells'][7]: del nb['cells'][7]['outputs']
if 'execution_count' in nb['cells'][7]: del nb['cells'][7]['execution_count']

# Patch Cell 6 (Data prep)
...
nb['cells'][6]['source'][-1] = nb['cells'][6]['source'][-1].strip('\n')

# Patch Cell 8 (ML eval)
